...
public class Innova {
	...
	public void punto1() {
		...
		for (int i = ventas.length - 8; i < ventas.length; i++) {
	...
	public  double[] suavizacionExponencialSimple(String producto, int index, ArrayList<ProductoDiario> lista) {
		if (lista.size() == 1) {
		...
		for (int i = 0; i < ventas.length; i++) {
			...
			for(String producto : mapProductos.keySet()) {
				...
				if(patron.equals("Horizontal")) {
					...
					System.out.println("\t\tLa cantidad de ventas será para un alfa de 0.3 y un beta de 0.7 son: "+(suavizacionDoble[0] < 0 ? ((((suavizacionDoble[0] * -1) *0.1)+ (tendencia[0]*0.9))/2) : suavizacionDoble[0])+" unidades. MAD = "+suavizacionDoble[1]);
				}
			}
